...
import { openScrollableSelect, type ScrollSelectConfig } from "../qna/scroll-select.js";
...
describe("openScrollableSelect", () => {
	...
	it("returns null when ui.select returns an unmatched label", async () => {
		const select = vi.fn(async () => "Gamma");
		const result = await openScrollableSelect(
			{ select },
			{
				title: "Fallback",
	...
	it("respects initialValue by starting cursor at the matching option", async () => {
